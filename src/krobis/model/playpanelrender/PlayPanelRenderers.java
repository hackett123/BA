...
public class PlayPanelRenderers {
  ...
  public static enum ShapeFillOption {
    ...
    private ShapeFillOption(BiConsumer<Graphics2D, Shape> render) {
      this.render = render;
    }
  ...
  public static enum ShapeCenteringOption {
    ...
    ALONG_BOTH((Shape shape) -> {
      ...
      Shape transformed = tr.createTransformedShape(shape);
      return transformed;
    }),
    ...
    private ShapeCenteringOption(UnaryOperator<Shape> adjust) {
      this.adjust = adjust;
    }
  ...
  public static class CenterText implements Renderable {
    ...
    public CenterText(String[] lines, int width, int[] yCoords, Font[] fonts) { 
      if ((lines.length != yCoords.length) || (yCoords.length != fonts.length)) {
        throw new IllegalArgumentException("Input arrays must have the same lengths!");
      }
      this.lines = lines;
      this.width = width;
